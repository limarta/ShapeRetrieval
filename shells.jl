...
begin
	bunny_1 = SR.load_obj("./meshes/shrec/16.obj")
	bunny_1 = SR.normalize_area(bunny_1)
	# bunny_1 = SR.normalize_mesh(bunny_1)
	
	bunny_2 = SR.load_obj("./meshes/shrec/17.obj")
	bunny_2 = SR.normalize_area(bunny_2)
	# bunny_2 = SR.normalize_mesh(bunny_2)
	...
	fig = SR.meshviz([bunny_1, bunny_2])
end
...
# ╔═╡ 9d5ee3cf-0e1b-4900-84d2-e70d182dd0ca
begin
	function smooth_shells(mesh_1,mesh_2,K)
		C = 0
		T = 0
		P = 0
		S_1 = SR.shell_coordinates(mesh_1, K[1], ϕ_1)
		S_2 = SR.shell_coordinates(mesh_2, K[1], ϕ_2)
		P, P_adj, C, T= SR.refine_correspondence(S_1,S_2,1)
		for k in K
			S_1 = SR.shell_coordinates(mesh_1, k, ϕ_1)
			S_2 = SR.shell_coordinates(mesh_2, k, ϕ_2)
			if k > 120
				P, P_adj, C, T= SR.refine_correspondence(S_1, S_2, P, P_adj,1; sigma=0.01)
			else
				P, P_adj, C, T= SR.refine_correspondence(S_1, S_2, P, P_adj,1; sigma=0.01)
			end
		end
	...
	K = 6
	P, P_adj, C, T = smooth_shells(bunny_1, bunny_2, [K,K+1,K+2,K+4,K+8,K+16])

	display(P)
	display(C)
	
	S_1 = SR.shell_coordinates(bunny_1, K+16, ϕ_1)
	S_2 = SR.shell_coordinates(bunny_2, K+16, ϕ_2)
	S_new = SR.apply_deformation(S_1, C,T)
	SR.meshviz([S_1, S_2, S_new])
end

# ╔═╡ 78597f2d-142f-4def-b894-ca5163649ebf
begin
	heat = 2*ceil.(bunny_2.V[3,:]*8)
	SR.meshviz(bunny_2, color=heat)
end
...
# ╠═f5dbd657-f4ce-4b3e-92f9-9e76f3a25e95
# ╟─013449b8-8c0f-48d1-adb2-7f839cd01fd8
# ╠═781b0cc9-3b72-4ac0-864e-062b0650ae87
# ╟─fcee8a90-c00b-43c6-9953-dd5b490e7024
# ╟─4be0504f-4b37-485f-a771-333aaa5024f6
# ╟─ea6875aa-fbbb-4ed9-a770-8708b78ca317
# ╠═9d5ee3cf-0e1b-4900-84d2-e70d182dd0ca
